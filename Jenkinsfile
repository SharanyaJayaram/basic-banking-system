    ...
    stage('Deploy Image') {
      steps{
          ...
          withCredentials([usernamePassword(credentialsId: 'dockerhub', passwordVariable: 'dockerhubPassword', usernameVariable: 'dockerhubUser')]) {
            ...
            sh "docker run -d -t -p 3000:3000 --name bankprojectcontainerrrrrr.. sharanyajayaram/bankdocker:latest"
          }
    ...
    stage('kubernetes') {
        steps {
            node("Kubes") {
              script{
                sh """
              #kubectl create deployment kubernetes-bootcamp1 --image=docker.io/sharanyajayaram/bankprojectcontainerrrrrr..:latest --port=8090
                       kubectl get pods """
